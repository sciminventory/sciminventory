...
async function requireVendorManager(organizationId: string, manage = true) {
  ...
  if (
    !membership ||
    !hasPermission(membership.role, manage ? "vendors.manage" : "vendors.read")
  )
    go("Your role does not have vendor management access.", "error");
...
export async function createVendor(formData: FormData) {
  ...
  if (!result.success)
    go(result.error.issues[0]?.message ?? "Check the vendor details.", "error");
  ...
  if (inserted.error || !inserted.data)
    go(inserted.error?.message ?? "Vendor could not be created.", "error");
  refresh();
  go("Vendor application created.", "success", inserted.data.id);
}

export async function updateVendorMasterData(formData: FormData) {
  const result = z
    .object({
      organizationId: id,
      supplierId: id,
  ...
  if (!result.success)
    go(
      result.error.issues[0]?.message ?? "Check the vendor information.",
      "error",
    );
  const data = result.data;
  ...
  await supabase.from("workflow_events").insert({
    ...
    to_status: "updated",
    message: "Supplier master information was updated.",
    actor_id: user.id,
  });
  refresh();
  go("Supplier information updated.", "success", data.supplierId);
}
...
export async function saveVendorContact(formData: FormData) {
  ...
  if (!result.success)
    go(
      result.error.issues[0]?.message ?? "Check the contact details.",
      "error",
    );
  const data = result.data;
  const contactId = data.contactId ? id.safeParse(data.contactId) : null;
  if (contactId && !contactId.success)
    go("Invalid vendor contact.", "error", data.supplierId);
  const { supabase } = await requireVendorManager(data.organizationId);
  const { error } = await supabase.rpc("manage_vendor_contact", {
    target_organization_id: data.organizationId,
    target_supplier_id: data.supplierId,
    target_contact_id: contactId?.data ?? null,
    contact_full_name: data.fullName,
    contact_email: data.email,
    contact_phone: data.phone,
    contact_job_title: data.jobTitle,
    contact_is_primary: data.isPrimary === "on",
    contact_is_active: data.isActive === "on",
  });
  if (error) go(error.message, "error", data.supplierId);
  refresh();
  go(
    data.contactId ? "Vendor contact updated." : "Vendor contact added.",
    "success",
    data.supplierId,
  );
}

export async function archiveVendorContact(formData: FormData) {
  const result = z
    .object({ organizationId: id, supplierId: id, contactId: id })
    .safeParse(Object.fromEntries(formData));
  if (!result.success) go("Invalid vendor contact.", "error");
  const data = result.data;
  const { supabase } = await requireVendorManager(data.organizationId);
  const { error } = await supabase.rpc("archive_vendor_contact", {
    target_organization_id: data.organizationId,
    target_supplier_id: data.supplierId,
  ...
  if (error) go(error.message, "error", data.supplierId);
  refresh();
  go("Vendor contact archived.", "success", data.supplierId);
}
...
export async function reviewVendor(formData: FormData) {
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      status: approved
        ? "active"
        : ["suspended", "archived", "rejected"].includes(data.onboardingStatus)
          ? "inactive"
          : "on_hold",
  ...
  await Promise.all([
    supabase.from("workflow_events").insert({
      ...
      entity_type: "vendor",
      entity_id: data.supplierId,
      from_status: current.data.onboarding_status,
      to_status: data.onboardingStatus,
    ...
    supabase.from("notifications").insert({
      ...
      supplier_id: data.supplierId,
      audience: "vendor",
      title: `Vendor review: ${data.onboardingStatus.replaceAll("_", " ")}`,
      message: data.notes || "Your vendor profile status was updated.",
      href: "/vendor/profile",
    }),
  ]);
  refresh();
...
export async function inviteVendorUser(formData: FormData) {
  const result = z
    .object({
      ...
      email: z.email(),
      role: z.enum(["admin", "member", "finance", "logistics"]),
    })
    .safeParse(Object.fromEntries(formData));
  if (!result.success)
    go(result.error.issues[0]?.message ?? "Check the invitation.", "error");
  if (!hasSupabaseAdminEnv())
    go(
      "Server invitation credentials are not configured.",
      "error",
  ...
  const invitation = await admin.auth.admin.inviteUserByEmail(
    ...
    {
      data: { full_name: data.fullName, account_type: "vendor" },
      redirectTo: `${origin}/auth/vendor-invite`,
    },
  );
  if (invitation.error || !invitation.data.user)
    go(
      invitation.error?.message ?? "Invitation could not be sent.",
      "error",
      data.supplierId,
    );
  const access = await admin.from("vendor_users").upsert(
    {
      organization_id: data.organizationId,
      supplier_id: data.supplierId,
    ...
    { onConflict: "organization_id,supplier_id,user_id" },
  );
  if (access.error)
    go(
      `Invitation was sent but vendor access failed: ${access.error.message}`,
      "error",
      data.supplierId,
    );
  refresh();
...
export async function addVendorCatalogItem(formData: FormData) {
  const result = z
    .object({
      ...
      leadTimeDays: z.coerce.number().int().min(0).max(3650),
      deliveryWindowDays: z.coerce.number().int().min(0).max(365),
      packagingSpecs: z.string().trim().max(1000),
    })
    .safeParse(Object.fromEntries(formData));
  if (!result.success)
    go(result.error.issues[0]?.message ?? "Check the catalog item.", "error");
  const data = result.data;
  const { supabase } = await requireVendorManager(data.organizationId);
  const { error } = await supabase.from("vendor_catalog_items").insert({
    ...
    vendor_item_code: data.itemCode,
    description: data.description,
    unit_price: data.unitPrice,
    currency: "PHP",
    minimum_order_quantity: data.moq,
    lead_time_days: data.leadTimeDays,
    delivery_window_days: data.deliveryWindowDays,
    packaging_specs: data.packagingSpecs || null,
    status: "active",
  });
  if (error) go(error.message, "error", data.supplierId);
  refresh();
...
export async function addPurchaseOrderLine(formData: FormData) {
  const result = z
    ...
    .safeParse(Object.fromEntries(formData));
  if (!result.success)
    go(result.error.issues[0]?.message ?? "Check the PO line.", "error");
  const data = result.data;
  const { supabase } = await requireVendorManager(data.organizationId);
  const lines = await supabase
    .from("procurement_record_lines")
    .select("line_number")
    .eq("procurement_record_id", data.purchaseOrderId)
    .order("line_number", { ascending: false })
    .limit(1);
  const { error } = await supabase.from("procurement_record_lines").insert({
    organization_id: data.organizationId,
    procurement_record_id: data.purchaseOrderId,
    product_id: data.productId || null,
    line_number: (lines.data?.[0]?.line_number ?? 0) + 1,
    description: data.description,
  ...
  const allLines = await supabase
    ...
    .eq("procurement_record_id", data.purchaseOrderId);
  const total = (allLines.data ?? []).reduce(
    (sum, line) =>
      sum +
  ...
  await supabase
    ...
    .update({ amount: total, currency: "PHP" })
...
export async function issuePurchaseOrder(formData: FormData) {
  const result = z
    .object({ organizationId: id, supplierId: id, purchaseOrderId: id })
    .safeParse(Object.fromEntries(formData));
  ...
  await Promise.all([
    supabase.from("workflow_events").insert({
      ...
      entity_id: data.purchaseOrderId,
    ...
    supabase.from("notifications").insert({
      ...
      supplier_id: data.supplierId,
      audience: "vendor",
      title: `New purchase order ${current.data.reference}`,
      message: "Review and acknowledge this purchase order.",
      href: "/vendor/orders",
    }),
  ]);
  refresh();
...
export async function generateReorderRequisitions(formData: FormData) {
  ...
  const { data, error } = await supabase.rpc("generate_reorder_requisitions", {
...
export async function recordGoodsReceipt(formData: FormData) {
  const result = z
    .object({
      ...
      qualityOk: z.string().optional(),
      conditionOk: z.string().optional(),
      packagingOk: z.string().optional(),
      rejectionReason: z.string().trim().max(1000),
    })
    .refine((value) => value.acceptedQuantity + value.rejectedQuantity > 0, {
      message: "Enter an accepted or rejected quantity.",
    })
    .safeParse(Object.fromEntries(formData));
  if (!result.success)
    go(result.error.issues[0]?.message ?? "Check the receipt.", "error");
  const data = result.data;
  const { supabase } = await requireVendorManager(data.organizationId);
  const { error } = await supabase.rpc("record_goods_receipt", {
    target_organization_id: data.organizationId,
    target_shipment_line_id: data.shipmentLineId,
    target_accepted_quantity: data.acceptedQuantity,
    target_rejected_quantity: data.rejectedQuantity,
    target_reference: data.reference,
    target_quality_ok: data.qualityOk === "on",
    target_condition_ok: data.conditionOk === "on",
    target_packaging_ok: data.packagingOk === "on",
    target_rejection_reason: data.rejectionReason || null,
  });
  if (error) go(error.message, "error", data.supplierId);
  refresh();
  go(
    "Goods receipt posted and accepted stock updated.",
    "success",
    data.supplierId,
  );
}
...
export async function matchVendorInvoice(formData: FormData) {
  const result = z
    .object({ organizationId: id, supplierId: id, invoiceId: id })
    .safeParse(Object.fromEntries(formData));
  ...
  const [po, lines] = await Promise.all([
    supabase
      .from("procurement_records")
      .select("amount")
      .eq("id", invoice.data.purchase_order_id)
      .single(),
    supabase
      .from("procurement_record_lines")
      .select("quantity,received_quantity,unit_price,tax_rate")
      .eq("procurement_record_id", invoice.data.purchase_order_id),
  ]);
  if (po.error || lines.error)
    go(
      po.error?.message ??
        lines.error?.message ??
  ...
  const receivedAmount = (lines.data ?? []).reduce(
    (sum, line) =>
      sum +
  ...
  const quantityVariance = (lines.data ?? []).reduce(
    (sum, line) => sum + Number(line.quantity) - Number(line.received_quantity),
    0,
  );
  const matched =
    Math.abs(priceVariance) <= 0.01 &&
    Math.abs(invoiceAmount - receivedAmount) <= 0.01 &&
    quantityVariance <= 0;
  const resultStatus = matched ? "matched" : "exception";
  const match = await supabase.from("invoice_matches").insert({
    organization_id: data.organizationId,
    invoice_id: data.invoiceId,
    purchase_order_amount: poAmount,
    ...
    result: resultStatus,
    details: {
      price_ok: Math.abs(priceVariance) <= 0.01,
      receipt_ok: Math.abs(invoiceAmount - receivedAmount) <= 0.01,
    },
    matched_by: user.id,
  });
  ...
  await supabase
    .from("vendor_invoices")
    .update({
      status: matched ? "approved" : "exception",
      mismatch_reason: matched
        ? null
        : "PO, received quantity, and invoice totals do not match.",
      approved_by: matched ? user.id : null,
      approved_at: matched ? new Date().toISOString() : null,
    })
    .eq("id", data.invoiceId);
  refresh();
  go(
    matched
      ? "Invoice passed three-way matching."
      : "Invoice exception created for review.",
    "success",
    data.supplierId,
  );
}
...
export async function recordVendorPayment(formData: FormData) {
  const result = z
    .object({
      ...
      status: z.enum(["pending", "approved", "scheduled", "paid"]),
    })
    .safeParse(Object.fromEntries(formData));
  if (!result.success)
    go(result.error.issues[0]?.message ?? "Check the payment.", "error");
  const data = result.data;
  const { supabase, user } = await requireVendorManager(data.organizationId);
  ...
  await supabase.from("notifications").insert({
    ...
    message: `${data.reference} was recorded for PHP ${data.amount.toFixed(2)}.`,
...
export async function calculateVendorPerformance(formData: FormData) {
  const result = z
    .object({ organizationId: id, supplierId: id })
    .safeParse(Object.fromEntries(formData));
  if (!result.success) go("Invalid vendor.", "error");
  const data = result.data;
  const { supabase } = await requireVendorManager(data.organizationId);
  const { error } = await supabase.rpc("refresh_vendor_performance", {
    target_organization_id: data.organizationId,
    target_supplier_id: data.supplierId,
  });
  if (error) go(error.message, "error", data.supplierId);
...
export async function addVendorReview(formData: FormData) {
  const result = z
    ...
    .safeParse(Object.fromEntries(formData));
  if (!result.success)
    go(result.error.issues[0]?.message ?? "Check the review.", "error");
  const data = result.data;
  const { supabase, user } = await requireVendorManager(data.organizationId);
  ...
  if (error) go(error.message, "error", data.supplierId);
  if (data.action === "suspend" || data.action === "deactivate")
    await supabase
      .from("suppliers")
      .update({
        status: "inactive",
        onboarding_status: data.action === "suspend" ? "suspended" : "archived",
      })
      .eq("id", data.supplierId);
  refresh();
...
export async function addVendorImprovementPlan(formData: FormData) {
  ...
  if (!result.success)
    go(
      result.error.issues[0]?.message ?? "Check the improvement plan.",
      "error",
    );
  const data = result.data;
  const { supabase, user } = await requireVendorManager(data.organizationId);
...
export async function createVendorConversation(formData: FormData) {
  const result = z
    .object({
      organizationId: id,
      supplierId: id,
      subject: z.string().trim().min(2).max(160),
      contextType: z.enum(["general", "purchase_order", "shipment", "invoice"]),
      contextId: z.string(),
      message: z.string().trim().min(1).max(4000),
    })
    .safeParse(Object.fromEntries(formData));
  if (!result.success) go("Check the conversation details.", "error");
  const data = result.data;
  const contextId =
    data.contextType === "general" ? null : id.safeParse(data.contextId);
  if (contextId && !contextId.success)
    goMessages(data.supplierId, "Select a valid transaction.", "error");
  const { supabase } = await requireVendorManager(data.organizationId, false);
  const { data: conversationId, error } = await supabase.rpc(
    "create_vendor_conversation",
    {
      target_organization_id: data.organizationId,
      target_supplier_id: data.supplierId,
      conversation_subject: data.subject,
      conversation_context_type: data.contextType,
      target_context_id: contextId?.data ?? null,
      initial_message: data.message,
    },
  );
  if (error) goMessages(data.supplierId, error.message, "error");
  refresh();
  goMessages(
    data.supplierId,
    "Conversation started and the vendor was notified.",
    "success",
    conversationId,
  );
}
...
export async function sendVendorMessage(formData: FormData) {
  const result = z
    ...
    .safeParse(Object.fromEntries(formData));
  if (!result.success) go("Enter a message.", "error");
  const data = result.data;
  const { supabase } = await requireVendorManager(data.organizationId, false);
  const { error } = await supabase.rpc("send_vendor_message", {
    target_conversation_id: data.conversationId,
    message_body: data.message,
  });
  if (error)
    goMessages(data.supplierId, error.message, "error", data.conversationId);
  refresh();
  goMessages(data.supplierId, "Message sent.", "success", data.conversationId);
}
