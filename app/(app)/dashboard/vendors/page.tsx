...
export default async function VendorsPage({
  ...
  const membership = await supabase
    ...
    .limit(1)
    .maybeSingle();
  if (!membership.data || !hasPermission(membership.data.role, "vendors.read"))
    redirect("/dashboard");
  ...
  const [
    ...
    supabase
      ...
      .order("created_at", { ascending: false }),
    supabase
      .from("vendor_improvement_plans")
      .select("*")
      .eq("organization_id", organizationId)
      .order("created_at", { ascending: false }),
    supabase
      .from("vendor_activity_history")
      .select("*")
      .eq("organization_id", organizationId)
      .order("occurred_at", { ascending: false })
      .limit(1000),
  ...
  const setupError = [
    ...
    conversations,
    messages,
  ].find((result) => result.error)?.error?.message;
  const securedPaths = [
    ...(documents.data ?? []).map((document) => document.storage_path),
    ...(invoices.data ?? []).flatMap((invoice) =>
      invoice.storage_path ? [invoice.storage_path] : [],
    ),
  ];
  const signedFiles = securedPaths.length
    ? await supabase.storage
        .from("organization-documents")
        .createSignedUrls(securedPaths, 600)
    : { data: [], error: null };
