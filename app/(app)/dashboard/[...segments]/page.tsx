...
export default async function OperationalModulePage({ params, searchParams }: { params: Promise<{ segments: string[] }>; searchParams: Promise<Search> }) {
  ...
  const [warehouseResult, productResult, supplierResult, locationResult, locationInventoryCheck] = await Promise.all([
...
async function loadItems(supabase: Client, module: OperationalModule, organizationId: string, warehouses: ModuleOption[], products: ModuleOption[], suppliers: ModuleOption[], locations: ModuleOption[]): Promise<OperationalItem[]> {
  ...
  if (module === "products") {
    ...
    return (data ?? []).map((row) => ({ ...item(row.id, row.sku, row.name, [row.category, row.unit_of_measure].filter(Boolean).join(" · "), row.is_active ? "active" : "inactive", row.reorder_point, null, row.created_at, null), editDetail: row.category ?? "" }));
  }
  if (module === "suppliers") {
  ...
  if (["requisitions", "rfqs", "quotations", "purchase_orders"].includes(module)) {
    ...
    return (data ?? []).map((row) => ({ ...item(row.id, row.reference, row.title, row.notes ?? (row.supplier_id ? supplierNames.get(row.supplier_id) ?? "Supplier" : "Internal request"), row.status, row.amount, row.due_at, row.created_at, row.warehouse_id ? warehouseNames.get(row.warehouse_id) ?? null : null), warehouseId: row.warehouse_id, relatedId: row.supplier_id }));
  }
  if (module === "logistics") {
