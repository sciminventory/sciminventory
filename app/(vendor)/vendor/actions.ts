...
async function requireVendor(
  section: Section,
  allowedRoles: VendorRole[] = ["admin", "member", "finance", "logistics"],
) {
  ...
  const access = await supabase
    .from("vendor_users")
    .select("organization_id,supplier_id,role,status")
    .eq("user_id", user.id)
    .eq("status", "active")
    .limit(1)
    .maybeSingle();
  if (!access.data) redirect("/dashboard");
  if (!allowedRoles.includes(access.data.role as VendorRole))
    go(section, "Your vendor role cannot perform this action.", "error");
...
export async function updateVendorProfile(formData: FormData) {
  ...
  if (!result.success)
    go(
      "profile",
      result.error.issues[0]?.message ?? "Check the profile.",
      "error",
    );
  const { supabase, access } = await requireVendor("profile", ["admin"]);
  const data = result.data;
  const { error } = await supabase.rpc("vendor_update_my_profile", {
    target_organization_id: access.organization_id,
    target_supplier_id: access.supplier_id,
    new_contact_email: data.contactEmail,
    new_phone: data.phone,
    new_website: data.website,
    new_address_line: data.address,
    new_city: data.city,
    new_province: data.province,
    new_postal_code: data.postalCode,
    new_delivery_capacity: data.deliveryCapacity,
    new_delivery_methods: split(data.deliveryMethods),
    new_service_areas: split(data.serviceAreas),
  });
...
export async function addVendorAddress(formData: FormData) {
  ...
  if (!result.success)
    go(
      "profile",
      result.error.issues[0]?.message ?? "Check the address.",
      "error",
    );
  const { supabase, access } = await requireVendor("profile", ["admin"]);
  const data = result.data;
  const { error } = await supabase.from("vendor_addresses").insert({
...
export async function addVendorShippingRule(formData: FormData) {
  const result = z
    .object({
      shippingMethod: z.string().trim().min(2).max(120),
      serviceArea: z.string().trim().min(2).max(160),
      cutoffTime: z.string(),
      delayPenaltyRate: z.coerce.number().min(0).max(100),
  ...
  if (!result.success)
    go(
      "profile",
      result.error.issues[0]?.message ?? "Check the shipping rule.",
      "error",
    );
  const { supabase, access } = await requireVendor("profile", ["admin"]);
  const data = result.data;
  const { error } = await supabase.from("vendor_shipping_rules").insert({
    ...
    delay_penalty_rate: data.delayPenaltyRate,
...
export async function addPortalCatalogItem(formData: FormData) {
  ...
  if (!result.success)
    go(
      "catalog",
      result.error.issues[0]?.message ?? "Check the catalog item.",
      "error",
    );
  const { supabase, access } = await requireVendor("catalog", [
    "admin",
    "member",
  ]);
  const data = result.data;
...
export async function uploadVendorDocument(formData: FormData) {
  ...
  if (!result.success)
    go(
      "profile",
      result.error.issues[0]?.message ?? "Check the document details.",
      "error",
    );
  const file = formData.get("file");
  if (!(file instanceof File) || file.size <= 0)
    go("profile", "Choose a document to upload.", "error");
  const validation = await validateVendorDocument(file);
  if (!validation.ok) go("profile", validation.message, "error");
  const { supabase, user, access } = await requireVendor("profile", [
    "admin",
    "member",
  ]);
  ...
  const inserted = await supabase.from("vendor_documents").insert({
    organization_id: access.organization_id,
    supplier_id: access.supplier_id,
  ...
  await supabase.from("notifications").insert({
    ...
    message: `${result.data.title} is ready for verification.`,
    href: `/dashboard/vendors?vendor=${access.supplier_id}`,
  });
  refresh();
...
export async function acknowledgePurchaseOrder(formData: FormData) {
  const result = z
    .object({
      ...
      response: z.enum(["confirmed", "revised", "rejected"]),
    ...
    .safeParse(Object.fromEntries(formData));
  if (!result.success)
    go("orders", "Check the acknowledgement details.", "error");
  const { supabase, access } = await requireVendor("orders", [
    "admin",
    "member",
  ]);
  const { error } = await supabase.rpc("acknowledge_purchase_order", {
    target_organization_id: access.organization_id,
    target_supplier_id: access.supplier_id,
    target_purchase_order_id: result.data.purchaseOrderId,
    target_response: result.data.response,
    target_proposed_amount: result.data.proposedAmount
      ? Number(result.data.proposedAmount)
      : null,
    target_proposed_delivery_date: result.data.proposedDeliveryDate || null,
    target_message: result.data.message || null,
  });
...
export async function createVendorShipment(formData: FormData) {
  const result = z
    .object({
      ...
      vehicleDetails: z.string().trim().max(300),
  ...
  if (!result.success)
    go(
      "shipments",
      result.error.issues[0]?.message ?? "Check the shipment.",
      "error",
    );
  const { supabase, user, access } = await requireVendor("shipments", [
    "admin",
    "logistics",
  ]);
  const data = result.data;
  ...
  const shipment = await supabase.from("shipments").insert({
    ...
    expected_arrival_at: data.expectedArrivalAt
      ? new Date(data.expectedArrivalAt).toISOString()
      : null,
    due_at: data.expectedArrivalAt
      ? new Date(data.expectedArrivalAt).toISOString()
      : null,
    status: "processing",
    created_by: user.id,
  ...
  const line = await supabase.from("shipment_lines").insert({
    ...
    purchase_order_line_id: data.purchaseOrderLineId,
    product_id: data.productId || null,
    description: data.description,
  ...
  await Promise.all([
    supabase.from("shipment_events").insert({
      ...
      shipment_id: shipmentId,
      status: "processing",
      message: "Shipment created by vendor.",
      recorded_by: user.id,
    }),
    supabase.from("notifications").insert({
      ...
      supplier_id: access.supplier_id,
      audience: "internal",
      title: `Shipment ${data.reference} created`,
      message: `The vendor is preparing ${data.quantity} units.`,
      href: `/dashboard/vendors?vendor=${access.supplier_id}`,
    }),
  ]);
  refresh();
...
export async function updateVendorShipment(formData: FormData) {
  const result = z
    ...
    .safeParse(Object.fromEntries(formData));
  if (!result.success)
    go("shipments", "Select a valid shipment update.", "error");
  const { supabase, user, access } = await requireVendor("shipments", [
    "admin",
    "logistics",
  ]);
  ...
  if (
    ...
    go("shipments", "Enter valid GPS coordinates.", "error");
  const shipment = await supabase
    .from("shipments")
    .update({
      status: data.status,
      dispatched_at:
        data.status === "dispatched" ? new Date().toISOString() : undefined,
    })
  ...
  await Promise.all([
    ...
    supabase.from("notifications").insert({
      ...
      supplier_id: access.supplier_id,
      audience: "internal",
      title: `Shipment ${data.status.replaceAll("_", " ")}`,
      message: data.message || "A vendor shipment was updated.",
      href: `/dashboard/vendors?vendor=${access.supplier_id}`,
    }),
  ]);
  refresh();
...
export async function submitVendorInvoice(formData: FormData) {
  ...
  if (!result.success)
    go(
      "invoices",
      result.error.issues[0]?.message ?? "Check the invoice.",
      "error",
    );
  const file = formData.get("file");
  if (!(file instanceof File) || file.size <= 0)
    go("invoices", "Attach the invoice PDF or DOCX file.", "error");
  const validation = await validateVendorDocument(file, true);
  if (!validation.ok) go("invoices", validation.message, "error");
  const { supabase, user, access } = await requireVendor("invoices", [
    "admin",
    "finance",
  ]);
  const data = result.data;
  ...
  const invoice = await supabase.from("vendor_invoices").insert({
    id: invoiceId,
    organization_id: access.organization_id,
    supplier_id: access.supplier_id,
    purchase_order_id: data.purchaseOrderId,
  ...
  const line = await supabase.from("vendor_invoice_lines").insert({
    organization_id: access.organization_id,
    invoice_id: invoiceId,
    purchase_order_line_id: data.purchaseOrderLineId || null,
    description: data.description,
    quantity: data.quantity,
    unit_price: data.unitPrice,
  ...
  await supabase.from("notifications").insert({
    ...
    supplier_id: access.supplier_id,
    audience: "internal",
    title: `Invoice ${data.invoiceNumber} submitted`,
    message: "Run three-way matching before payment approval.",
    href: `/dashboard/vendors?vendor=${access.supplier_id}`,
  });
  refresh();
...
export async function createPortalConversation(formData: FormData) {
  const result = z
    ...
    .safeParse(Object.fromEntries(formData));
  if (!result.success)
    go("messages", "Check the conversation details.", "error");
  const { supabase, access } = await requireVendor("messages");
  const data = result.data;
  const contextId =
    data.contextType === "general" ? null : uuid.safeParse(data.contextId);
  if (contextId && !contextId.success)
    go("messages", "Select a valid transaction.", "error");
  const { error } = await supabase.rpc("create_vendor_conversation", {
    target_organization_id: access.organization_id,
    target_supplier_id: access.supplier_id,
  ...
  if (error) go("messages", error.message, "error");
  refresh();
  go("messages", "Conversation started and the procurement team was notified.");
}
...
export async function sendPortalMessage(formData: FormData) {
  const result = z
    ...
    .safeParse(Object.fromEntries(formData));
  if (!result.success) go("messages", "Enter a message.", "error");
  const { supabase } = await requireVendor("messages");
  const { error } = await supabase.rpc("send_vendor_message", {
    target_conversation_id: result.data.conversationId,
    message_body: result.data.message,
  });
  if (error) go("messages", error.message, "error");
  refresh();
  go("messages", "Message sent.");
}
