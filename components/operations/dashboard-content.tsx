      ...
      <ScrollReveal distance={14}>
        <header className="flex flex-wrap items-end justify-between gap-5">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-bold tracking-[-.05em] sm:text-4xl">Good morning, {userName}.</h1>
        ...
        <div className="grid gap-5 xl:grid-cols-[1.18fr_.82fr]">
          ...
          <section className="rounded-2xl border border-line bg-white shadow-[0_14px_45px_rgba(25,72,133,.055)]">
            ...
            <div className="p-6">
              ...
              <div className="mt-6 grid grid-cols-2 gap-4">{healthRows.map(([label, value, tone]) => <div key={label} className="rounded-xl border border-line p-3"><div className="flex items-center gap-2"><span className={`size-2.5 rounded-full ${tone}`} /><span className="text-[10px] font-semibold text-muted">{label}</span><span className="ml-auto font-mono text-[9px] text-muted">{healthTotal ? Math.round((value / healthTotal) * 100) : 0}%</span></div><strong className="mt-2 block text-lg">{value.toLocaleString()}</strong></div>)}</div>
      ...
      <ScrollReveal className="mt-5" distance={18}>
        <div className="grid gap-5 md:grid-cols-2">
          <section className="rounded-2xl border border-line bg-white p-6 shadow-[0_14px_45px_rgba(25,72,133,.055)]">
            <div className="flex items-center justify-between"><div><h2 className="text-base font-bold">Recent movements</h2><p className="mt-1 text-[10px] text-muted">Latest immutable ledger activity</p></div><Link href="/dashboard/inventory/movements" className="text-[10px] font-bold text-blue-700">Open ledger</Link></div>
            <div className="mt-5">{data.recentMovements.length ? data.recentMovements.map((movement) => { const incoming = movement.quantity > 0; return <div key={`${movement.reference}-${movement.type}`} className="grid grid-cols-[40px_1fr_auto] items-center gap-3 border-t border-line py-4 first:border-0"><span className={`grid size-10 place-items-center rounded-xl ${incoming ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-blue-700"}`}>{incoming ? <ArrowDownLeft size={15} /> : <ArrowUpRight size={15} />}</span><div><p className="text-[11px] font-semibold">{humanize(movement.type)}</p><p className="mt-1 font-mono text-[9px] text-muted">{movement.reference}</p></div><span className="font-mono text-[11px] font-medium">{movement.quantity > 0 ? "+" : ""}{formatQuantity(movement.quantity)}</span></div>; }) : <EmptyState text="No stock movements have been posted yet." compact />}</div>
          </section>
...
function SectionHeading({ title, description, action }: { title: string; description: string; action: React.ReactNode }) { return <div className="flex items-center justify-between gap-4 border-b border-line px-6 py-5"><div><h2 className="text-base font-bold">{title}</h2><p className="mt-1 text-[10px] text-muted">{description}</p></div>{action}</div>; }
