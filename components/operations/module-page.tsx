...
const fieldClass = "h-10 w-full rounded-lg border border-line bg-white px-3 text-xs outline-none transition focus:border-accent focus:ring-3 focus:ring-blue-100";
    ...
    <div className="min-w-0 w-full p-3 min-[380px]:p-4 sm:p-6 lg:p-8 xl:p-10">
      ...
      <header className="flex flex-wrap items-end justify-between gap-5">
        ...
        {config.readOnly ? (
          ...
          <span className="rounded-full border border-line bg-white px-3 py-2 font-mono text-[9px] uppercase text-muted">{props.role.replaceAll("_", " ")} · read only</span>
        )}
      </header>
      </ScrollReveal>

      {(props.success || props.error || props.setupError) && (
        <div role="status" className={`mt-6 flex items-start gap-3 rounded-xl border px-4 py-3 text-xs ${props.error || props.setupError ? "border-red-200 bg-red-50 text-red-800" : "border-blue-200 bg-blue-50 text-blue-800"}`}>
            ...
            >
              <div className="flex items-start gap-3 border-b border-line bg-white px-4 py-4 sm:gap-4 sm:px-6 sm:py-5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-[10px] text-muted">Validated and securely scoped to your organization.</p>
                </div>
...
function CreateForm({ config, organizationId, warehouses, products, suppliers }: { config: ModuleConfig; organizationId: string; warehouses: ModuleOption[]; products: ModuleOption[]; suppliers: ModuleOption[] }) {
  ...
  return (
    <form action={createOperationalItem} className="flex min-h-0 flex-1 flex-col">
      ...
      {config.key === "documents" && <label className="sm:col-span-2"><span className={labelClass}>File · max 25 MB</span><input required type="file" name="file" className="block h-10 w-full rounded-lg border border-line bg-white text-[10px] file:mr-3 file:h-full file:border-0 file:border-r file:border-line file:bg-blue-50 file:px-3 file:text-[10px] file:font-bold file:text-blue-700" /></label>}
      </div>
      <div className="border-t border-line bg-white p-5">
        ...
        <p className="mt-3 text-center text-[9px] text-muted">This action will be recorded in the workspace audit history.</p>
      </div>
    </form>
  );
}
...
function ItemRow({ item, config, organizationId, canManage }: { item: OperationalItem; config: ModuleConfig; organizationId: string; canManage: boolean }) {
  return (
    <tr className="border-b border-line text-[11px] last:border-0">
      ...
      <td className="max-w-xs px-5 py-4"><p className="font-bold">{item.title}</p><p className="mt-1 truncate text-[9px] text-muted">{item.detail || "No additional details"}</p></td>
      <td className="px-5 py-4 text-[10px] text-muted">{item.warehouse ?? "Organization-wide"}</td>
...
function Metric({ label, value, detail }: { label: string; value: string; detail: string }) { return <div className="border-b border-r border-line p-5"><p className="text-[10px] font-semibold text-muted">{label}</p><p className="mt-3 text-2xl font-bold tracking-[-.04em]">{value}</p><p className="mt-1 text-[9px] text-muted">{detail}</p></div>; }
function StatusBadge({ status }: { status: string }) { const danger = ["cancelled", "rejected", "exception", "inactive"].includes(status); const done = ["completed", "delivered", "verified", "active"].includes(status); return <span className={`rounded-full px-2 py-1 font-mono text-[8px] ${danger ? "bg-red-50 text-red-700" : done ? "bg-blue-50 text-blue-700" : "bg-amber-50 text-amber-800"}`}>{humanize(status).toUpperCase()}</span>; }
