...
import { RESUME_FILE_ACCEPT } from "@/lib/recruitment/resume-files";
...
const fieldClass = "h-11 w-full rounded-xl border border-line bg-white px-3 text-sm outline-none transition focus:border-accent focus:ring-3 focus:ring-blue-100";
    ...
    <div className="min-w-0 w-full p-3 min-[380px]:p-4 sm:p-6 lg:p-8 xl:p-10">
      <ScrollReveal distance={14}>
        <header className="flex flex-wrap items-end justify-between gap-5">
          <div className="min-w-0"><div className="flex flex-wrap items-center gap-3"><p className="eyebrow text-muted">{copy.eyebrow}</p><RecruitmentRealtimeSync organizationId={props.organizationId} /></div><div className="mt-2 flex items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-blue-50 text-blue-700 sm:size-11"><Icon size={20} /></span><h1 className="break-words text-2xl font-bold tracking-[-.05em] sm:text-3xl">{copy.title}</h1></div><p className="mt-3 text-sm text-muted">{copy.description}</p></div>
          {props.section === "screening" && props.canRunScreening ? <a href="#run-screening" className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-accent px-4 text-sm font-bold text-white sm:w-auto"><Sparkles size={15} />{copy.action}</a> : canCreate ? <button type="button" onClick={openDrawer} className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-accent px-4 text-sm font-bold text-white sm:w-auto"><Plus size={15} />{copy.action}</button> : <span className="inline-flex h-10 items-center gap-2 rounded-xl border border-line bg-white px-3 text-sm font-semibold text-muted"><ShieldCheck size={14} /> Read-only access</span>}
        </header>
      </ScrollReveal>

      {(props.success || props.error || props.setupError) && <div className={`mt-6 flex items-start gap-3 rounded-xl border px-4 py-3 text-sm ${props.error || props.setupError ? "border-red-200 bg-red-50 text-red-800" : "border-blue-200 bg-blue-50 text-blue-800"}`}>{props.error || props.setupError ? <CircleAlert size={15} /> : <CheckCircle2 size={15} />}<span>{props.error ?? props.setupError ?? props.success}</span></div>}

      <ScrollReveal className="mt-6" distance={16}><div className="grid grid-cols-2 overflow-hidden rounded-2xl border border-line bg-white shadow-[0_14px_40px_rgba(25,72,133,.055)] sm:grid-cols-4"><Metric label="Open jobs" value={props.jobs.filter((job) => job.status === "open").length} /><Metric label="Applicants" value={props.applicants.length} /><Metric label="Active pipeline" value={props.applications.filter((application) => !["hired", "rejected", "withdrawn"].includes(application.stage)).length} /><Metric label="Hired" value={props.applications.filter((application) => application.stage === "hired").length} /></div></ScrollReveal>
...
function Jobs({ jobs, organizationId, canManage }: { jobs: RecruitmentJob[]; organizationId: string; canManage: boolean }) {
  if (!jobs.length) return <Panel><Empty text="No job openings yet." /></Panel>;
  return (
    <Panel>
        ...
        {jobs.map((job) => (
          <article key={job.id} className="p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0"><p className="break-all font-mono text-sm text-blue-700">{job.reference}</p><h2 className="mt-1 break-words text-sm font-bold">{job.title}</h2><p className="mt-1 text-sm text-muted">{job.department} · {job.location ?? "Flexible"}</p></div>
              {!canManage && <Tag>{humanize(job.status)}</Tag>}
            </div>
            <div className="mt-4 rounded-xl bg-mist p-3 text-sm"><p>{humanize(job.employment_type)} · {job.min_years_experience} years · {humanize(job.education_level)}</p><div className="mt-2 flex flex-wrap gap-1">{job.required_skills.slice(0, 4).map((skill) => <Tag key={skill}>{skill}</Tag>)}</div><p className="mt-3 font-mono text-xs text-muted">Created {formatDate(job.created_at)}</p></div>
            {canManage && <JobStatusForm job={job} organizationId={organizationId} mobile />}
          </article>
        ))}
      </div>
      <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[900px] text-left"><TableHead labels={["Job", "Department", "Requirements", "Created", "Status"]} /><tbody>{jobs.map((job) => <tr key={job.id} className="border-b border-line text-sm last:border-0"><td className="px-5 py-4"><p className="font-mono text-xs text-blue-700">{job.reference}</p><p className="mt-1 font-bold">{job.title}</p><p className="mt-1 text-xs text-muted">{humanize(job.employment_type)} · {job.location ?? "Flexible"}</p></td><td className="px-5 py-4">{job.department}</td><td className="max-w-xs px-5 py-4"><p>{job.min_years_experience} years · {humanize(job.education_level)}</p><div className="mt-2 flex flex-wrap gap-1">{job.required_skills.slice(0, 4).map((skill) => <Tag key={skill}>{skill}</Tag>)}</div></td><td className="px-5 py-4 font-mono text-xs text-muted">{formatDate(job.created_at)}</td><td className="px-5 py-4">{canManage ? <JobStatusForm job={job} organizationId={organizationId} /> : <Tag>{humanize(job.status)}</Tag>}</td></tr>)}</tbody></table></div>
    </Panel>
  );
}

function JobStatusForm({ job, organizationId, mobile = false }: { job: RecruitmentJob; organizationId: string; mobile?: boolean }) {
  return <form action={updateJobStatus} className={`flex gap-2 ${mobile ? "mt-4 w-full" : ""}`}><input type="hidden" name="organizationId" value={organizationId} /><input type="hidden" name="jobId" value={job.id} /><select name="status" defaultValue={job.status} className={`h-10 min-w-0 rounded-lg border border-line px-2 text-sm ${mobile ? "flex-1" : ""}`}>{["draft", "open", "paused", "closed", "archived"].map((status) => <option key={status}>{status}</option>)}</select><AdminActionButton variant="secondary" className="h-10">Save</AdminActionButton></form>;
}
...
function Pipeline({ applications, organizationId, canManage }: { applications: RecruitmentApplication[]; organizationId: string; canManage: boolean }) {
  return <div className="grid gap-4 pb-3 sm:auto-rows-fr sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">{stages.map((stage) => { const rows = applications.filter((application) => application.stage === stage); return <section key={stage} className="flex min-w-0 flex-col rounded-2xl border border-line bg-white shadow-sm sm:h-full sm:min-h-[240px]"><div className="flex items-center justify-between border-b border-line px-4 py-3"><h2 className="text-sm font-bold">{humanize(stage)}</h2><span className="rounded-full bg-blue-50 px-2 py-1 font-mono text-xs text-blue-700">{rows.length}</span></div><div className="flex-1 space-y-3 p-3">{rows.map((application) => <article key={application.id} className="rounded-xl border border-line p-4 shadow-sm"><h3 className="break-words text-sm font-bold">{application.applicant_name}</h3><p className="mt-1 break-words text-xs text-muted">{application.job_title}</p><div className="mt-3 flex flex-wrap gap-1">{application.declared_skills.slice(0, 3).map((skill) => <Tag key={skill}>{skill}</Tag>)}</div>{canManage && <form action={updateApplicationStage} className="mt-4 flex gap-2"><input type="hidden" name="organizationId" value={organizationId} /><input type="hidden" name="applicationId" value={application.id} /><select name="stage" defaultValue={stage} className="h-10 min-w-0 flex-1 rounded-lg border border-line px-2 text-xs">{stages.map((option) => <option key={option}>{option}</option>)}</select><AdminActionButton variant="secondary" className="h-10">Move</AdminActionButton></form>}</article>)}{!rows.length && <p className="px-2 py-8 text-center text-xs text-muted">No applications</p>}</div></section>; })}</div>;
}
...
function Screening({ jobs, runs, scores, organizationId, canRun }: { jobs: RecruitmentJob[]; runs: ScreeningRun[]; scores: RecruitmentScore[]; organizationId: string; canRun: boolean }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[360px_1fr]"><section id="run-screening" className="scroll-mt-24 rounded-2xl border border-line bg-white p-5 shadow-sm"><span className="grid size-11 place-items-center rounded-xl bg-blue-50 text-blue-700"><BrainCircuit size={20} /></span><h2 className="mt-4 text-base font-bold">Rank applications</h2><p className="mt-2 text-sm leading-5 text-muted">The model compares job content with resume text and structured qualifications. Results support recruiter review; they do not make hiring decisions.</p>{canRun ? <form action={runApplicantScreening} className="mt-5 space-y-4"><input type="hidden" name="organizationId" value={organizationId} /><Select label="Job opening" name="jobId" options={jobs.map((job) => ({ value: job.id, label: `${job.reference} · ${job.title}` }))} /><AdminActionButton className="h-11 w-full"><Sparkles size={14} /> Run screening</AdminActionButton></form> : <p className="mt-5 rounded-xl border border-line bg-mist p-4 text-sm text-muted">Your role can review existing results but cannot start a screening run.</p>}<div className="mt-5 rounded-xl border border-blue-100 bg-blue-50 p-4"><p className="text-sm font-bold text-blue-800">Responsible use</p><p className="mt-2 text-xs leading-5 text-blue-700">Identity attributes are excluded. A human must review evidence, accommodations, and job relevance before changing a hiring stage.</p></div></section><section className="overflow-hidden rounded-2xl border border-line bg-white shadow-sm"><div className="border-b border-line px-5 py-4"><h2 className="text-sm font-bold">Latest ranking</h2><p className="mt-1 text-xs text-muted">{latest ? `${latest.model_name} · version ${latest.model_version} · ${formatDate(latest.completed_at ?? latest.created_at)}` : "No completed screening run"}</p></div>{scores.length ? <div className="divide-y divide-line">{scores.map((score) => <article key={score.id} className="grid gap-4 p-5 lg:grid-cols-[44px_1fr_90px]"><span className="grid size-11 place-items-center rounded-xl bg-ink text-sm font-bold text-white">#{score.rank}</span><div><h3 className="text-sm font-bold">{score.applicant_name}</h3><p className="mt-1 text-xs text-muted">{score.job_title}</p><div className="mt-3 flex flex-wrap gap-1">{score.matched_skills.map((skill) => <Tag key={skill}>{skill}</Tag>)}{score.missing_skills.slice(0, 3).map((skill) => <span key={skill} className="rounded-full bg-red-50 px-2 py-1 text-xs text-red-700">Missing {skill}</span>)}</div><div className="mt-4 grid grid-cols-4 gap-2">{[["Semantic", score.semantic_score], ["Skills", score.skills_score], ["Experience", score.experience_score], ["Education", score.education_score]].map(([label, value]) => <div key={String(label)}><p className="text-xs text-muted">{label}</p><div className="mt-1 h-1.5 overflow-hidden rounded-full bg-blue-50"><span className="block h-full rounded-full bg-blue-500" style={{ width: `${value}%` }} /></div><p className="mt-1 font-mono text-xs">{Number(value).toFixed(0)}</p></div>)}</div></div><div className="text-right"><strong className="text-2xl text-blue-700">{score.overall_score.toFixed(1)}</strong><p className="text-xs uppercase text-muted">fit score</p></div></article>)}</div> : <Empty text="Run screening to generate an explainable ranking." />}</section></div>;
}

function RecruitmentDrawer({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) { return <><motion.button type="button" aria-label="Close form" onClick={onClose} className="fixed inset-0 z-[70] bg-ink/40 backdrop-blur-[2px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} /><motion.aside role="dialog" aria-modal="true" className="fixed inset-y-0 right-0 z-[80] flex h-dvh w-full flex-col bg-[#f8faff] shadow-[-24px_0_70px_rgba(7,23,45,.18)] sm:max-w-[620px]" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 320, damping: 32 }}><div className="flex items-center gap-3 border-b border-line bg-white px-4 py-4 sm:px-6 sm:py-5"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-blue-50 text-blue-700"><Plus size={17} /></span><h2 className="min-w-0 break-words text-lg font-bold sm:text-xl">{title}</h2><button type="button" onClick={onClose} className="ml-auto grid size-10 shrink-0 place-items-center rounded-xl border border-line"><X size={17} /></button></div>{children}</motion.aside></>; }
...
function Metric({ label, value }: { label: string; value: number }) { return <div className="border-b border-r border-line p-5"><p className="text-sm text-muted">{label}</p><p className="mt-2 text-2xl font-bold">{value}</p></div>; }
function TableHead({ labels }: { labels: string[] }) { return <thead><tr className="border-b border-line bg-[#f8faff] font-mono text-xs uppercase text-muted">{labels.map((label) => <th key={label} className="px-5 py-3 font-medium">{label}</th>)}</tr></thead>; }
...
function TextArea({ label, ...props }: React.TextareaHTMLAttributes<HTMLTextAreaElement> & { label: string }) { return <label className="sm:col-span-2"><span className={labelClass}>{label}</span><textarea className="min-h-36 w-full resize-y rounded-xl border border-line bg-white p-3 text-sm outline-none focus:border-accent focus:ring-3 focus:ring-blue-100" {...props} /></label>; }
...
function FormFooter({ label }: { label: string }) { return <div className="border-t border-line bg-white p-5"><AdminActionButton className="h-11 w-full">{label}</AdminActionButton><p className="mt-3 text-center text-xs text-muted">Applicant and hiring changes are recorded in the audit history.</p></div>; }
function Tag({ children }: { children: React.ReactNode }) { return <span className="rounded-full bg-blue-50 px-2 py-1 text-xs text-blue-700">{children}</span>; }
function Empty({ text }: { text: string }) { return <div className="grid min-h-56 place-items-center p-8 text-center"><div><FileText className="mx-auto text-blue-300" size={24} /><p className="mt-3 text-sm text-muted">{text}</p></div></div>; }
