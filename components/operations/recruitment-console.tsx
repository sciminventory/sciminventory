    ...
    <div className="min-w-0 w-full p-3 min-[380px]:p-4 sm:p-6 lg:p-8 xl:p-10">
      <ScrollReveal distance={14}>
        <header className="flex flex-wrap items-end justify-between gap-5">
          ...
          {props.section === "screening" && props.canRunScreening ? <a href="#run-screening" className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-accent px-4 text-[11px] font-bold text-white sm:w-auto"><Sparkles size={15} />{copy.action}</a> : canCreate ? <button type="button" onClick={openDrawer} className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-accent px-4 text-[11px] font-bold text-white sm:w-auto"><Plus size={15} />{copy.action}</button> : <span className="inline-flex h-10 items-center gap-2 rounded-xl border border-line bg-white px-3 text-[10px] font-semibold text-muted"><ShieldCheck size={14} /> Read-only access</span>}
        </header>
      </ScrollReveal>
...
function Pipeline({ applications, organizationId, canManage }: { applications: RecruitmentApplication[]; organizationId: string; canManage: boolean }) {
  return <div className="hide-scrollbar flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3">{stages.map((stage) => { const rows = applications.filter((application) => application.stage === stage); return <section key={stage} className="w-[min(285px,calc(100vw-48px))] shrink-0 snap-start rounded-2xl border border-line bg-white shadow-sm"><div className="flex items-center justify-between border-b border-line px-4 py-3"><h2 className="text-xs font-bold">{humanize(stage)}</h2><span className="rounded-full bg-blue-50 px-2 py-1 font-mono text-[9px] text-blue-700">{rows.length}</span></div><div className="space-y-3 p-3">{rows.map((application) => <article key={application.id} className="rounded-xl border border-line p-4 shadow-sm"><h3 className="break-words text-xs font-bold">{application.applicant_name}</h3><p className="mt-1 break-words text-[9px] text-muted">{application.job_title}</p><div className="mt-3 flex flex-wrap gap-1">{application.declared_skills.slice(0, 3).map((skill) => <Tag key={skill}>{skill}</Tag>)}</div>{canManage && <form action={updateApplicationStage} className="mt-4 flex gap-2"><input type="hidden" name="organizationId" value={organizationId} /><input type="hidden" name="applicationId" value={application.id} /><select name="stage" defaultValue={stage} className="h-10 min-w-0 flex-1 rounded-lg border border-line px-2 text-[9px]">{stages.map((option) => <option key={option}>{option}</option>)}</select><AdminActionButton variant="secondary" className="h-10">Move</AdminActionButton></form>}</article>)}{!rows.length && <p className="px-2 py-8 text-center text-[9px] text-muted">No applications</p>}</div></section>; })}</div>;
}
