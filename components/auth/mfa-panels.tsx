...
export function MfaSetupPanel({ required = false, verifiedFactors = [], nextPath = "/dashboard" }: { required?: boolean; verifiedFactors?: MfaFactor[]; nextPath?: string }) {
  ...
  async function verifyEnrollment() {
    ...
    router.replace(required ? nextPath : "/dashboard/security");
    router.refresh();
  }
...
export function MfaChallengePanel({ factorId, nextPath = "/dashboard" }: { factorId: string; nextPath?: string }) {
  const router = useRouter();
  const [code, setCode] = useState("");
  const [error, setError] = useState("");
  const [busy, setBusy] = useState(false);
